# -------------------------------------------------------------------------
# Upload the data and make a bar plot
#
# -------------------------------------------------------------------------
...
type_df <- melt(dcast(data[, c("CLOUD_MASK_TYPE_VALID", "TIMEPOINT")], TIMEPOINT ~ CLOUD_MASK_TYPE_VALID), 
                id = c("TIMEPOINT"))
...
type_nms   <- c("0", "cirrus_-20", "cirrus_-50", "cirrus_-50_small",
...
cloud_group <- c("cirrus_-20", "cirrus_-50", "cirrus_-50_small",
...
type_df$cloud_mask_type_valid <- group

# type_df2 <- type_df # with cloud_mask_type only

#type_df2 <- type_df[which(type_df$group != "other"), ]

ggplot(data = type_df, aes(x = date, y = n_pixels, fill = cloud_mask_type_valid)) + 
  geom_bar(stat = 'identity') + ggtitle("ahi_NT_10_minute_2015_1Sep_23Sep_20190130")
